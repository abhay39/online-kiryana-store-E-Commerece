...
<body>
  <div class="h-[100vh] flex flex-col  md:flex-row ">
    ...
    <div class="flex justify-center w-full items-center  p-6 space-x-5 space-y-5">
      ...
      <form method="POST" action="/checkLogin" class="bg-slate-300 p-5 rounded">
        @csrf
       ...
       <div class="mt-2">
        ...
        <br />
        <input value="{{old('email')}}" name="email"  placeholder="Enter your email id" class="p-2 rounded border-green-300 outline-none sm:w-[100%] md:w-[250px]"/>
        @error('email')
      ...
      @enderror
       ...
       <div class="mt-2">
        ...
        <br />
        <input value="{{old('password')}}"  name="password" type="password" placeholder="Enter your password" class="p-2 rounded border-green-300 outline-none sm:w-[100%] md:w-[250px]"/>
        @error('password')
        <p class="shadow-sm text-red-600">{{$message}}</p>
      @enderror
       </div>
